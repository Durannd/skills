...
**Rendering Strategy (SvelteKit):**

- **Server-Side Rendering (SSR)** → Default for SEO and initial load
- **Client-Side Rendering (CSR)** → For highly interactive islands
- **Hydration** → Managed by SvelteKit automatically
- **Server Actions/Form Actions** → Standard way to handle mutations
...
### Svelte Ecosystem (Svelte 5)

- **Runes**: $state, $derived, $effect, $props, $inspect
- **Components**: Snippets (`{#snippet}`), Render tags (`{@render}`)
- **Data Flow**: SvelteKit Load functions, parallel fetches
- **Structure**: `+page.svelte`, `+layout.svelte`, `+server.ts`
